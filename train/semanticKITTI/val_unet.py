...
from SpConv_squeeze.core import suqeeze_model_v13_5 as suqeeze_model
from SpConv_squeeze.dataset import data_muti_frame_val as data
from SpConv_squeeze.utils import np_ioueval
from SpConv_squeeze.utils import utils
from SpConv_squeeze.config import *
...
out_dit = os.path.join(data.data_base,'val_muti_v13_5')
    ...
    # if scn.is_power2(epoch):
        with torch.no_grad():
            ...
            for rep in range(1,1+data.val_reps):
                # times = time.time()
                evealer.reset()
                for i,batch in tqdm.tqdm(enumerate(data.get_val_data_loader()),total= len(data.val)//(batch_size)):#
                    ...
                    for bid,id in enumerate(idx):
                        ...
                        evealer.addBatch(pre_prediction, batch['y'][start_y:start_y+lp[bid]].numpy())  # TLabel
